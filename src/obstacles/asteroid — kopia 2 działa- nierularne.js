...
void main() {
  ...
  // Obliczamy kąt dla obecnego piksela (od -PI do PI)
  float angle = atan(uv.y, uv.x);
  
  // Tworzymy szum na okręgu. Używamy uSeed, aby każda skała była inna
  float shapeNoise = fbm(vec2(cos(angle), sin(angle)) * 2.5 + uSeed * 3.0);
  
  // Zmienny promień krawędzi (bazowy promień 0.75 + do 0.25 z szumu)
  float edgeLimit = 0.75 + 0.25 * shapeNoise; 

  // Wycinamy piksele poza naszym nowym, nieregularnym kształtem
  if (d > edgeLimit) discard;

  // Numerycznie liczona pseudo-normalna
  float eps = 0.015;
  ...
  // Kolor skały + ciemne żyłki/spękania
  ...
  // Delikatne przyciemnienie sylwetki przy krawędzi dostosowane do nowego limitu
  float rimShade = smoothstep(edgeLimit, edgeLimit - 0.25, d);
