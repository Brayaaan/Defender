...
function showFotoCrashScreen() {
    ...
    // Rysowanie napisu "Rozbiłeś się" (NOWY KOD)
    textSize(32); // Można użyć większej czcionki, by był bardziej widoczny
    fill(255); // Kolor biały (tak jak "Naciśnij Spację")
    textAlign(CENTER); // Wyśrodkowanie
    text("Rozbiłeś się", width / 2, boxY - 40); // 40 pikseli nad górną krawędzią kontenera
    ...
    textAlign(CENTER);
    text("Naciśnij Spację", width / 2, boxY + boxSize + 20); // Tekst pod ramką
}
...
function showGameOverScreen() {
  ...
  // 1. Obliczenie środka pionowego (Y) kontenera
  let topBoxY = margin;
  let topBoxCenterY = topBoxY + topBoxHeight / 2;
  // Używamy width / 2 jako center X

  noFill();
  stroke(0, 0, 255); // Niebieska ramka
  strokeWeight(2);
  rect(width / 2 - topBoxWidth / 2, topBoxY, topBoxWidth, topBoxHeight, 20); // Zaokrąglone wierzchołki

  fill(173, 216, 230); // Bardzo jasnoniebieski dla tekstu
  noStroke();
  textAlign(CENTER);

  // 2. Wiersz 1: Koniec gry!
  textSize(0.04 * width);
  // Przesunięcie w górę od środka kontenera (dla lepszego wizualnego odstępu od drugiego wiersza)
  let yKoniecGry = topBoxCenterY - topBoxHeight * 0.18; 
  text("Koniec gry!", width / 2, yKoniecGry); 

  // 3. Wiersz 2: Twój wynik
  textSize(0.0208 * width);
  // Przesunięcie w dół od środka kontenera
  let yTwojWynik = topBoxCenterY + topBoxHeight * 0.18; 
  text(`Twój wynik: ${score}`, width / 2, yTwojWynik);
  ...
  let leftText1 = "Naciśnij spację aby kontynuować od tego poziomu";
  ...
  textAlign(CENTER);
  text("Najlepsi piloci:", width / 2, topBoxHeight + 2 * margin + 0.02 * height);

  textAlign(LEFT);
  for (let i = 0; i < Math.min(highScores.length, 14); i++) { // Ograniczono do 14
    let yPos = topBoxHeight + 2 * margin + 0.07 * height + i * 0.05 * height; // Wrócono do pierwotnego odstępu
    if (i === 1) {
      drawBurningLine(margin, margin + leftBoxWidth + 2 * margin + 10, yPos - 0.025 * height + 2, rightBoxWidth - 20); // Kreska podniesiona o 10 pikseli
    }
    if (i < 3) { // Animacja dla pierwszych trzech miejsc
      ...
      drawAnimatedFireball(margin + leftBoxWidth + 2 * margin + 10, yPos + 7, frameCount + frameCountOffset); // Kula przed imieniem
      text(nameText, margin + leftBoxWidth + 2 * margin + 20, yPos); // Numer i imię z 3 spacjami
      drawAnimatedFireball(margin + leftBoxWidth + 2 * margin + textWidth(nameText) + 20, yPos + 7, frameCount + frameCountOffset); // Kula po imieniu
      if (i === 1 || i === 2) {
        stroke(255, 0, 0); // Czerwone podkreślenie
        strokeWeight(1);
        line(margin + leftBoxWidth + 2 * margin + 10, yPos + 20, margin + leftBoxWidth + 2 * margin + rightBoxWidth - 30, yPos + 20); // Podkreślenie na całą szerokość
        noStroke();
      }
...
function showWinScreen() {
    ...
    // Rysowanie napisu "Gratulacje, ukończyłeś wszystkie poziomy !!!"
    textSize(32); // Można użyć większej czcionki, by był bardziej widoczny
    fill(255); // Kolor biały (tak jak "Naciśnij Spację")
    textAlign(CENTER); // Wyśrodkowanie
    text("Gratulacje, ukończyłeś wszystkie poziomy !!!", width / 2, boxY - 40); // 40 pikseli nad górną krawędzią kontenera

    // Tu dodajemy kod do rysowania zdjęcia wewnątrz ramki
    if (window.galaktykaLoaded) {
      // Rysowanie natywnym Canvas API
      let ctx = drawingContext;
      // Wpasowanie obrazka w ramkę z 20-pikselowym marginesem
      ctx.drawImage(window.galaktykaImage, boxX + 10, boxY + 10, boxSize - 20, boxSize - 20);
    } else {
      // Alternatywny tekst lub kształt, gdy obraz nie jest załadowany
      fill(255);
      textSize(24);
      textAlign(CENTER);
      text("Nieudane Ładowanie obrazu galaktyki...", width / 2, height / 2);
    }
...
function handleEndScreensKeyPress() {
    ...
    if ((gameState === 'gameOver' || gameState === 'win') && keyCode === 32) {
        ...
        // JEST TO MIEJSCE DO ZAPISANIA PUNKTÓW PRZED PRZEJŚCIEM DO EKRANU PUNKTÓW PRZY PRZEJŚCIU CAŁEJ GRY
        if (gameState === 'win') {
             saveHighScore(); // <--- WYWOŁANIE ZAPISU JEDYNIE DLA STANU ZWYCIĘSTWA
        }
    ...
    // Akcje dla ekranu z tabelą wyników (gameState === 'scoreScreen')
        ...
        if (keyCode === 32) {
            ...
            resetGame();
            gameState = 'start';
            playEchoMusic(); // Nowa linia, aby muzyka włączyła się raz
            loop();
